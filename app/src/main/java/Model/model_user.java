### Model_User.java – User Model

`Model_User` is responsible for managing **user authentication and registration** in the Safety Drones application using **Firebase Authentication** and **Firebase Realtime Database**.

Key responsibilities:
- Validates user input during registration (name, email, and password).
- Registers new users using Firebase Authentication.
- Stores additional user information in Firebase Realtime Database.
- Sends an email verification link after successful registration.
- Handles user login using:
  - Email and password authentication.
  - Google Sign-In authentication.
- Provides Google Sign-In configuration through `GoogleSignInOptions`.
- Retrieves basic drone statistics for the user, such as:
  - Total drones registered
  - Last drone added
  - Last drone edited.

The class uses **callback interfaces** to handle asynchronous login and registration operations.
