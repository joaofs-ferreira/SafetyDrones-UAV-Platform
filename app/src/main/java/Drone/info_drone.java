### InfoDrone.java
This class displays **detailed information about a selected drone** and provides options to **edit or delete** it.  

Key responsibilities:
- Receives **drone data via Intent** and populates UI fields (name, model, weight, autonomy, speed, category).  
- Dynamically updates **category details** and corresponding images based on drone classification (C0–C4).  
- Provides **buttons linking to YouTube videos** for practical guidance and information per category.  
- Handles **edit action** by launching `EditDrone` with pre-filled data.  
- Handles **delete action** with Firebase integration to remove the drone from the database.  
- Returns results to the previous fragment/activity upon edits or deletion.  
- Uses **Toast messages** to inform the user of success or failure.
