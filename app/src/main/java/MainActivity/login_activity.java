### LoginActivity.java – User Authentication Activity

`LoginActivity` is responsible for handling **user authentication** within the Safety Drones application, allowing users to log in using **email/password or Google authentication**.

Key responsibilities:
- Provides input fields for **email and password** using `EditText`.
- Validates user input:
  - Ensures fields are **not empty**.
  - Verifies that the **email format is valid**.
- Uses **`Model_User`** to manage authentication logic following the **MVC architecture**.
- Authenticates users with **Firebase Authentication** using:
  - **Email and password login**
  - **Google Sign-In integration**
- Displays **Toast messages** to inform users of login success or errors.
- Redirects successfully authenticated users to the **main application page (`FirstPage`)**.
- Provides navigation to the **registration screen (`RegisterActivity`)** for new users.
- Uses an **ActivityResultLauncher** to handle the Google authentication result.

Overall, `LoginActivity` acts as the **secure login interface of the application**, ensuring proper credential validation and providing multiple authentication methods for user convenience.
