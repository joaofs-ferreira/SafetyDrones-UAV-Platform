...
`FirstPage` is the **main entry point of the app**, handling the **bottom navigation** and fragment switching for the different modules of the Safety Drones app.

Key responsibilities:
- Uses a **BottomNavigationView** to navigate between modules:  
  - `Tempo` – Weather information  
  - `Maps` – Map overview  
  - `Config` – User configuration  
  - `Drones` – Drone management  
  - `Home` – Main home screen  
- Delegates navigation logic to the **`FirstPageController`**, following the **MVC pattern**.  
- Dynamically **shows or hides fragments** depending on the selected menu item:  
  - Home content is shown directly in the main layout.  
  - Other modules are loaded in a `FrameLayout` as fragments.  
- Maintains **fragment back stack** to support proper navigation when switching modules.  
- Uses **`updateViewBasedOnSelection`** to update the UI according to the selected item ID.  
- Ensures the **default selected tab is Home** on first launch.  

Overall, `FirstPage` acts as the **central navigation hub** of the app, providing smooth transitions between all major sections while keeping the home content accessible.
