...
This class manages the **interactive map for drone zones**, integrating Google Maps to display restricted, authorized, or prohibited areas.  

Key responsibilities:
- Initializes a **Google Map** and sets the camera over Portugal by default.  
- Loads **zone data from a local JSON file** (`UASZoneVersion.json`) into a `Model_Zona` via `MapaDroneController`.  
- Provides **filtering options** through a BottomSheetDialog:  
  - All zones (`TODAS`)  
  - Prohibited zones (`PROHIBITED`)  
  - Zones requiring authorization (`REQ_AUTHORISATION`)  
  - Zones with no restrictions (`NO_RESTRICTION`)  
- Adds zones to the map as **circles or polygons** with colored markers based on restriction type.  
- Handles **marker clicks** to show detailed zone information in an AlertDialog.  
- Tracks **zone visit statistics** in `SharedPreferences`:  
  - Total zones displayed  
  - Number of visits per zone  
  - Last visited zone  
- Updates the map dynamically when filters are applied.  
- Displays **loading animations** while map data is processed.  
- Logs warnings for **missing or malformed zone data**, ensuring robustness.  
- Supports a **back button** to exit the activity.  

Overall, `MapaDrone` provides a **visual, interactive interface for drone pilots** to understand operational restrictions and plan flights safely.
