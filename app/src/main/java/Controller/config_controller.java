...
This class handles the **business logic for user settings** in the app.  
It connects the UI fragments and activities with the Firebase backend and the local models.

Key responsibilities:
- **Load and display user data** (email, name, profile image, account creation date).  
- **Manage profile image updates** (local Base64 or external URL).  
- **Edit user name** and synchronize changes with Firebase.  
- **Handle user logout** and redirect to the login screen.  
- **Reset password requests** via Firebase Authentication.  
- **Load statistics** for drones and zones.  
- **Delete user account**, with proper reauthentication for email/password or Google login.  
- Interacts with **Firebase Realtime Database**, **FirebaseAuth**, and **GoogleSignIn** to manage secure user operations.
    ...
    public void mostrarDialogoConfirmacaoEliminacao(FirebaseUser user, String email) {
        ...
        if (isGoogleLogin) {
            ...
            builder.setPositiveButton("Eliminar", (dialog, which) -> {
                delete_account_google_ou_email(email, null); 
            });
        } else {
